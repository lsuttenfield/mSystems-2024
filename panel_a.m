...
V1(:,i)=P1(:,2*Nl+i);
   ...
   %% phase 2 time
Tend2 =28.8-Tend1;
% Initial Conditions first L5 and L6 then L1 and L2 at 10 time units
P02 = [F(1:2).*(1-PHI(1:2))*1e-4 zeros(1,2)  P1(end,5) P1(end,Nl) zeros(1,4) P1(end,Nl+5) P1(end,2*Nl) zeros(1,4) P1(end,2*Nl+5) P1(end,3*Nl)]; 
...
TT=28.8*7;
Tend3 =TT-Tend2;
% Initial Conditions first L5 and L6, then L1 and L2 at 10 time units then
% L3 and L4 after one day
P03 = [L(end,1) L(end,2) F(3:4).*(1-PHI(3:4))*1e-4 L(end,5) L(end,Nl) IQ(end,1) IQ(end,2) zeros(1,2) IQ(end,5) IQ(end,Nl) V(end,1) V(end,2) zeros(1,2) V(end,5) V(end,Nl)]; 
